...
foreach ($entradas as $entrada) {
    ...
    $blogEntryId = $entrada['blog_entrie_id'];
    $imagePath = $entrada['image_path'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    if (!empty($_FILES['file']['name'])) {
        $file = $_FILES['file'];
        $imagePath = $blogController->subirImagen($file);
    ...
    $actualizada = $blogController->actualizarEntrada($blogEntryId, $title, $content, $imagePath);
...
<body style="background-color: #3a5a40">
    ...
    <div class="page">
        ...
        <div class="row my-5">
            <section class="mt-5" style="margin: 0 auto;">
                <div class="container mt-5">
                    <div class="row mt-5">
                        <div class="card shadow">
                            <h2 class="card-title">Editando
                                <?= $title; ?>...
                            </h2>
                            <div class="card-body">
                                <form action="#" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-input" type="file" name="file" data-constraints="@file"
                                            accept=".png,.jpg,.jpeg,.gif" />
                                    </div>
                                    <div class="form-group">
                                        <label for="contact-content">
                                            Nuevo contenido</label>
                                        <textarea required class="form-input" name="content"
                                            data-constraints="@Required"><?= $content; ?></textarea>
                                    </div>
                                    <button type="submit" name="guardar" value="guardar" class="btn btn-primary">Guardar
                                        Cambios</button>
                                </form>
                            </div>
                        </div>
